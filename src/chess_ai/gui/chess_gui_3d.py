...
class SimpleChessGUI3D:
    ...
    def get_3d_offset(self, file: int, rank: int) -> Tuple[int, int]:
        ...
        x_offset = int((rank - 3.5) * math.sin(math.radians(self.camera.rotation_y)) * depth_factor * 10)
        y_offset = int((rank - 3.5) * math.sin(math.radians(self.camera.rotation_x)) * depth_factor * 10)
    ...
    def screen_to_board(self, screen_pos: Tuple[int, int]) -> Optional[int]:
        ...
        if (x < board_start_x or x >= board_start_x + self.BOARD_SIZE or
            y < board_start_y or y >= board_start_y + self.BOARD_SIZE):
            return None
    ...
    def draw_board(self):
        ...
        for rank in range(8):
            for file in range(8):
                ...
                rect = pygame.Rect(x + x_offset, y + y_offset, self.CELL_SIZE, self.CELL_SIZE)
                pygame.draw.rect(self.screen, color, rect)
                pygame.draw.rect(self.screen, (0, 0, 0), rect, 1)
                
                # Ombre pour l'effet de profondeur
                if x_offset != 0 or y_offset != 0:
                    shadow_rect = pygame.Rect(x + 2, y + 2, self.CELL_SIZE, self.CELL_SIZE)
    ...
    def draw_piece(self, piece: chess.Piece, square: int):
        ...
        # Couleur de la pièce
        color = self.COLORS['white_piece'] if piece.color else self.COLORS['black_piece']
    ...
    def handle_click(self, pos: Tuple[int, int]):
        ...
        if self.selected_square is None:
            ...
            if piece and piece.color == self.environment.board.turn:
                ...
                print(f"✅ Pièce sélectionnée: {piece} ({len(self.possible_moves)} mouvements)")
            else:
            ...
            if square == self.selected_square:
                ...
                # Vérifier promotion
                piece = self.environment.board.piece_at(self.selected_square)
                if (piece and piece.piece_type == chess.PAWN and 
                    (chess.square_rank(square) == 7 or chess.square_rank(square) == 0)):
                    move.promotion = chess.QUEEN
                
                try:
                    if self.environment.make_move(move):
                        print(f"✅ Mouvement: {chess.square_name(self.selected_square)} → {chess.square_name(square)}")
                        self.selected_square = None
                        self.possible_moves = []
                    else:
                ...
                if piece and piece.color == self.environment.board.turn:
                    ...
                    print(f"✅ Nouvelle sélection: {piece} ({len(self.possible_moves)} mouvements)")
                else:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.dragging_camera:
                        rel = (event.pos[0] - self.last_mouse_pos[0], 
